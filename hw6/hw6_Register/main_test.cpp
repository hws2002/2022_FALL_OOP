...
void encrypt_username(vector<string>&res,string temp){
    istringstream ss(temp);
    string buff;
    getline(ss,buff, ':');
    getline(ss,buff, ':');
...
void encrypt_password(vector<string>&res,string temp){
    istringstream ss(temp);
    string buff;
    getline(ss,buff, ':');
    getline(ss,buff, ':');
...
void encrypt_email(vector<string>&res,string temp){
    istringstream ss(temp);
    string buff;
    getline(ss,buff, ':');
    getline(ss,buff, ':');
...
그럼 각각 
*/
void replace_username(string pre_name){
    // std::vector<char> encrypted_name(pre_name.begin(), pre_name.end());
...
void replace_password(string pre_pass){
    regex re("[a-zA-z]");
    Info.insert({"password",regex_replace(pre_pass,re,"*")});
}

void replace_email(string pre_email){
    regex re("[a-zA-Z_0-9]");
    Info.insert({"email",regex_replace(pre_email,re,"*")});
}
...
int main(){
    ...
    while(getline(ss,temp,',')){
        switch (check_type(temp))
        {
            case 0 :
                ...
                else encrypt_username(res,temp);
                can_Register[0] = can;
                break;
                }
            case 1 :
                ...
                else encrypt_password(res,temp);
                can_Register[1] = can;
                break;
                }
            case 2 :
    ...
    if(can_register){
        replace_username(res[0]);
        replace_password(res[1]);
        replace_email(res[2]);
        cout<<"Successfully registered."<<endl;
        cout<<"username: "<<Info["username"]<<endl;
        cout<<"password: "<<Info["password"];
